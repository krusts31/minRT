...
{
	if (hit_dig == 1)
		return (fl_parce(&(*sq)->cor->e[0], vars));
	if (hit_dig == 2)
		return (fl_parce(&(*sq)->cor->e[1], vars));
	if (hit_dig == 3)
		return (fl_parce(&(*sq)->cor->e[2], vars));
	if (hit_dig == 4)
		return (fl_parce(&(*sq)->norm->e[0], vars));
	if (hit_dig == 5)
		return (fl_parce(&(*sq)->norm->e[1], vars));
	if (hit_dig == 6)
		return (fl_parce(&(*sq)->norm->e[2], vars));
	if (hit_dig == 7)
		return (fl_parce(&(*sq)->side_size, vars));
	if (hit_dig == 8)
		return (char_parce(&(*sq)->col->e[0], vars));
	if (hit_dig == 9)
		return (char_parce(&(*sq)->col->e[1], vars));
	if (hit_dig == 10)
		return (char_parce(&(*sq)->col->e[2], vars));
	return (1);
}
