...
{
	if (hit_dig == 1)
		return (fl_parce(&(*cy)->cor->e[0], vars));
	if (hit_dig == 2)
		return (fl_parce(&(*cy)->cor->e[1], vars));
	if (hit_dig == 3)
		return (fl_parce(&(*cy)->cor->e[2], vars));
	if (hit_dig == 4)
		return (fl_parce(&(*cy)->norm->e[0], vars));
	if (hit_dig == 5)
		return (fl_parce(&(*cy)->norm->e[1], vars));
	if (hit_dig == 6)
		return (fl_parce(&(*cy)->norm->e[2], vars));
	if (hit_dig == 7)
		return (fl_parce(&(*cy)->diameter, vars));
	if (hit_dig == 8)
		return (fl_parce(&(*cy)->height, vars));
	if (hit_dig == 9)
		return (char_parce(&(*cy)->col->e[0], vars));
	if (hit_dig == 10)
		return (char_parce(&(*cy)->col->e[1], vars));
	if (hit_dig == 11)
		return (char_parce(&(*cy)->col->e[2], vars));
	return (1);
}
